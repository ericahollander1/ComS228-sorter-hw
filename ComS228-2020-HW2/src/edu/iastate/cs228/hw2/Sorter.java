...
public abstract class Sorter
{
  ...
  public void sortWithStatistics(WordList toSort, Comparator<String> comp, int totalToSort) throws NullPointerException, IllegalArgumentException{
    // TODO
	  WordList copy = toSort.clone();
	  CountingComparator countComp = new CountingComparator(comp);
	  int i;
	  long afterTime = 0; //time stored before I sort
	  long beforeTime = 0; //time stored after I sort
	  for(i= 0; i < totalToSort; i += toSort.length()) {
		  if(i ==0) {
			  beforeTime = System.currentTimeMillis();
			  sort(copy, countComp);
			  afterTime = System.currentTimeMillis();
			  totalSortingTime += (afterTime - beforeTime);
		  }
		  else {
			  beforeTime = System.currentTimeMillis();
			  sort(toSort.clone(), countComp);
			  afterTime = System.currentTimeMillis();
			  totalSortingTime += (afterTime - beforeTime);
		  }
	  }
	  totalComparisons = countComp.getCount();
	  totalWordsSorted = i;
	  
	  FileWriter writer = null;
	  File f = null;
	  
	  //f = new File(getName() + ".txt");
	  try {
		f = new File(getName() + ".txt");
